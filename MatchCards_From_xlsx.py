...
def create_time_range(time, noon):
    hr_min_list = time.split(":")
    mins = int(hr_min_list[0])*60 + int(hr_min_list[1])+30
    hr = mins//60
    min = mins - hr*60
    hr = str(hr)
    min = str(min)
    ...
    if int(hr) >12:
        hr = "1"
        noon = "PM"
    end_time = str(hr) +":"+ str(min)+noon
    return time + noon + " - "+ end_time

def draw_match_card(canvas, x, y, match):
    # Draw the match name, date, and time
    time = match[1].value
    timelist = time.split(" ")
    time = create_time_range(timelist[2], timelist[3])
    event = match[2].value
    round = match[6].value
    name = match[7].value + " vs. " + match[8].value
    ...
    for i in range(len(teamslist)):
        if teamslist[i].split(" ")[0] == "Winner" or teamslist[i].split(" ")[0] == "Loser":
            teamslist[i] = teamslist[i].split(" ")[0] + teamslist[i].split(" ")[1]
        elif teamslist[i].split(" ")[0] == "If":
            teamslist[i] = teamslist[i].split(" ")[1] + teamslist[i].split(" ")[2]
...
# display the contents of the sheet
print(worksheet)

# Create a new PDF document
pdf = canvas.Canvas('match_cards_from_xl.pdf', pagesize=letter)
...
total_cards = worksheet.max_row-5
